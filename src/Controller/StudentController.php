...
class StudentController extends AbstractController
{
    ...
    public function __construct(EntityManagerInterface $em, StudentsRepository $studentsRepository, SchoolclassRepository $schoolclassRepository) {
        $this->em = $em;
    ...
    public function show($studentid): Response
    {
        ...
        foreach ($trimesterData as $trimester => $skills) {
            ...
            $averageStudentMark[$trimester] = $averageMark ? $averageMark : 0;
        ...
        $averageYearMark = count($averageStudentMark) > 0 ? round(array_sum($averageStudentMark) / count($averageStudentMark), 2) : null;


    $labels = ['year', 1, 2, 3];
    $colors = [
        'rgba(255, 99, 132, 0.5)' => 'rgb(255, 99, 132)',
        'rgba(72, 143, 51, 0.5)' => 'rgb(72, 143, 51)',
        'rgba(88, 74, 213, 0.5)' => 'rgb(88, 74, 213)',
        'rgba(255, 219, 39, 0.5)' => 'rgb(255, 219, 39)',
    ];
    $datasetLabels = [
        'year' => 'Récap annuel',
        1 => 'Trimestre 1',
    ...
    $chartOptions = [
        'scales' => [
            'r' => [
                'angleLines' => [
                    'display' => false, // Keeps the angle lines off
                ],
                'grid' => [
                    'color' => 'rgba(0, 0, 0, 0.6)', // Darker grid lines (black with 80% opacity)
        ...
        return $this->render('student/showstudent.html.twig', [
            ...
            'averageStudentMark' => $averageStudentMark
        ]);
    }
    ...
    #[Route('/print/{classid}', name: 'printRecap', methods: ['GET'])]
public function printRecap(int $classid): Response
{
    ...
    $isDarkBackground = $this->schoolclassRepository->isDarkBackground($selectedClass->getColor());
    ...
    foreach ($students as $student) {
        ...
        // Organize StudentTests by trimester and store test details
        foreach ($studentTests as $studentTest) {
            $trimester = $studentTest->getTest()->getTrimester();

            // Collect test details
            $testDetails = [
                'test_id' => $studentTest->getId(),
                'mark' => $studentTest->getMark(),
                'scale' => $studentTest->getTest()->getScale(),
                'coefficient' => $studentTest->getTest()->getCoefficient(),
                'skills' => [
                    'skill1' => $studentTest->getSkill1(),
                    'skill2' => $studentTest->getSkill2(),
                    'skill3' => $studentTest->getSkill3(),
                    'skill4' => $studentTest->getSkill4(),
                    'skill5' => $studentTest->getSkill5(),
                ]
            ];
            $studentTestDetails[$trimester][] = $testDetails;

            // Aggregate data for averages calculation
            if (!isset($trimesterData[$trimester])) {
                $trimesterData[$trimester] = [
                    'skill1' => [],
                    'skill2' => [],
        ...
        return $this->render('class/print.html.twig', [
            'students' => $students,
